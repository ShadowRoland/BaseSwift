...
extension SRIndexPath {
    open class Form: SRIndexPath.Table {
        open weak var titleView: UIView? //左边标题控件
        open var title = "" //左边标题控件的文字内容
        open weak var inputTextView: UIView? //右边内容的可输入控件，一般为UITextField或者UITextView
        open var inputText = "" //右边输入控件的文字内容
        open var placeholder = "" //右边输入控件的placeholder，仅限textField
        open weak var showTextView: UIView? //右边内容的只读控件，可以与inputTextView同为一个控件，也可以另为UILabel
        open var showText = "" //右边只读控件的文字内容
        
        open var isRequired = false //字段值是否必填
        
        open var inputRegex: String? //右边输入控件通过键盘或粘贴时输入内容的正则表达式
        open var textRegex: String? //右边输入控件所有文字内容的正则表达式
        open var textRegexDescription: String? //右边输入控件最终文字内容的正则表达式的描述，用于出错时的提示
        open var textMaxLength = 0 //右边输入控件所有文字内容的最大长度，不大于0时表示无限制
    }
}
