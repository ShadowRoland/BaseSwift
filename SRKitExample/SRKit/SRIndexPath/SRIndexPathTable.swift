...
extension SRIndexPath {
    open class Table: SRIndexPath.Item {
        ...
        open var height = C.tableCellHeight
        ...
        open var isIgnoreParamValue = false //在数据字典中读取数据时，忽略情景：获取不到正确的value时（包括1、paramKey为nil；2、字典不存在为paramKey的key；3、字典中paramKey对应的value为NSNull；4、字典中paramKey对应的value类型不正确）
        open var paramValueType: JsonValueType = .string //对应数据字典的value的类型
        open var paramKey: String? //对应数据字典的key
        open var paramValue: Any? //对应数据字典的value，目前的要求是必须可以转化为json字典中的value
