...
public class SRIndexPath {
    public class Set: NSObject {
        private(set) var set: Swift.Set<SRIndexPath.Item> = []
        
        public var enumerated: EnumeratedSequence<Swift.Set<SRIndexPath.Item>> {
            return set.enumerated()
        }
        
        //将所有的items根据indexPath的大小顺序输出为数组
        public var sorted: [SRIndexPath.Item] {
            return enumerated.sorted { $0.element.indexPath < $1.element.indexPath }.map { $0.element }
        }
        
        public func append(_ item: SRIndexPath.Item?) {
        ...
        public func remove(_ item: SRIndexPath.Item?) {
            guard let item = item else { return }
            set.remove(item)
        }
        ...
        public subscript(key: Any) -> SRIndexPath.Item? {
            get {
                if key is IndexPath {
    ...
    public class Item: NSObject {
        ...
        public var config: [AttributedString.Key : Any] = [:]
