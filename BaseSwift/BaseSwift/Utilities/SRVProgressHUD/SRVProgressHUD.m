...
@property (nonatomic, weak) UIView *foregroundHudView;
@property (nonatomic, strong) UIView *flatHudView;

@end

@implementation SRVProgressHUD {
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        ...
        /*
         ...
         UIImage* infoImage = [UIImage imageWithContentsOfFile:[imageBundle pathForResource:@"info"
         ofType:@"png"]];
         UIImage* successImage = [UIImage imageWithContentsOfFile:[imageBundle
         pathForResource:@"success" ofType:@"png"]];
         UIImage* errorImage = [UIImage imageWithContentsOfFile:[imageBundle
         pathForResource:@"error" ofType:@"png"]];
        ...
        self.accessibilityIdentifier = @"SRVProgressHUD";
...
- (void)updateHUDFrame {
    ...
    if (self.statusLabel.text) {
        ...
        labelRect = [self.statusLabel.text
        boundingRectWithSize:constraintSize
                     options:(NSStringDrawingOptions)(NSStringDrawingUsesFontLeading | NSStringDrawingTruncatesLastVisibleLine |
    ...
    if (imageUsed || progressUsed) {
        ...
        contentHeight =
        CGRectGetHeight(imageUsed ? self.imageView.frame : self.indefiniteAnimatedView.frame);
    }
    ...
    // self.hudView.bounds = CGRectMake(0.0f, 0.0f, MAX(self.minimumSize.width, hudWidth),
    //                                 MAX(self.minimumSize.height, hudHeight));
    self.foregroundHudView.bounds = CGRectMake(0.0f, 0.0f, MAX(self.minimumSize.width, hudWidth),
                                               MAX(self.minimumSize.height, hudHeight));
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000
    ...
    [self.foregroundHudView.superview bringSubviewToFront:self.foregroundHudView];
    ...
    if (self.statusLabel.text) {
        CGFloat yOffset =
        MAX(SVProgressHUDVerticalSpacing,
            (self.minimumSize.height - contentHeight - SVProgressHUDLabelSpacing - labelHeight) / 2.0f);
        centerY = yOffset + contentHeight / 2.0f;
    } else {
        // centerY = CGRectGetMidY(self.hudView.bounds);
        centerY = CGRectGetMidY(self.foregroundHudView.bounds);
    }
    // self.indefiniteAnimatedView.center = CGPointMake(CGRectGetMidX(self.hudView.bounds),
    // centerY);
    self.indefiniteAnimatedView.center = CGPointMake(CGRectGetMidX(self.foregroundHudView.bounds), centerY);
    if (self.progress != SVProgressHUDUndefinedProgress) {
        self.backgroundRingView.center = self.ringView.center =
        // CGPointMake(CGRectGetMidX(self.hudView.bounds), centerY);
        CGPointMake(CGRectGetMidX(self.foregroundHudView.bounds), centerY);
    }
    // self.imageView.center = CGPointMake(CGRectGetMidX(self.hudView.bounds), centerY);
    self.imageView.center = CGPointMake(CGRectGetMidX(self.foregroundHudView.bounds), centerY);

    // Label
    if (imageUsed || progressUsed) {
        centerY = CGRectGetMaxY(imageUsed ? self.imageView.frame : self.indefiniteAnimatedView.frame) +
                  SVProgressHUDLabelSpacing + labelHeight / 2.0f;
    } else {
        // centerY = CGRectGetMidY(self.hudView.bounds);
        centerY = CGRectGetMidY(self.foregroundHudView.bounds);
    }
    ...
    self.statusLabel.center = CGPointMake(CGRectGetMidX(self.foregroundHudView.bounds), centerY);
...
#if TARGET_OS_IOS
- (void)updateMotionEffectForOrientation:(UIInterfaceOrientation)orientation {
    UIInterpolatingMotionEffectType xMotionEffectType =
    UIInterfaceOrientationIsPortrait(orientation) ? UIInterpolatingMotionEffectTypeTiltAlongHorizontalAxis :
                                                    UIInterpolatingMotionEffectTypeTiltAlongVerticalAxis;
    UIInterpolatingMotionEffectType yMotionEffectType =
    UIInterfaceOrientationIsPortrait(orientation) ? UIInterpolatingMotionEffectTypeTiltAlongVerticalAxis :
                                                    UIInterpolatingMotionEffectTypeTiltAlongHorizontalAxis;
    [self updateMotionEffectForXMotionEffectType:xMotionEffectType
...
- (void)positionHUD:(NSNotification *)notification {
    ...
    // No transforms applied to window in iOS 8, but only if compiled with iOS 8 SDK as base SDK,
    // otherwise system supports old rotation logic.
...
- (void)moveToPoint:(CGPoint)newCenter rotateAngle:(CGFloat)angle {
    ...
    if (self.containerView) {
        self.hudView.center = CGPointMake(self.containerView.center.x + self.offsetFromCenter.horizontal,
                                          self.containerView.center.y + self.offsetFromCenter.vertical);
    } else {
...
- (void)showProgress:(float)progress status:(NSString *)status {
    __weak SRVProgressHUD *weakSelf = self;
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        __strong SRVProgressHUD *strongSelf = weakSelf;
        if (strongSelf) {
            ...
            if (progress >= 0) {
                ...
                if (!strongSelf.ringView.superview) {
                    ...
                    if (!strongSelf.flatHud) {
                        [strongSelf.hudVibrancyView.contentView addSubview:strongSelf.ringView];
                    } else {
                        [strongSelf.flatHudView addSubview:strongSelf.ringView];
                    }
#else
                ...
                if (!strongSelf.backgroundRingView.superview) {
                    ...
                    if (!strongSelf.flatHud) {
                        ...
                        [strongSelf.flatHudView addSubview:strongSelf.backgroundRingView];
                    }
#else
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000
                //[strongSelf.hudVibrancyView.contentView
                //addSubview:strongSelf.indefiniteAnimatedView];
                if (!strongSelf.flatHud) {
                    [strongSelf.hudVibrancyView.contentView addSubview:strongSelf.indefiniteAnimatedView];
                } else {
                    [strongSelf.flatHudView addSubview:strongSelf.indefiniteAnimatedView];
                }
#else
...
- (void)showStatus:(NSString *)status {
    ...
    //[self.hudView setNeedsDisplay];
    [self.foregroundHudView setNeedsLayout];
    ...
    if (self.defaultMaskType != SVProgressHUDMaskTypeNone) {
        ...
        self.foregroundHudView.accessibilityLabel = status;
        self.foregroundHudView.isAccessibilityElement = YES;
    }
...
// Checking one alpha value is sufficient as they are all the same
    ...
    if ((_flatHud ? self.flatHudView.alpha : self.hudView.contentView.alpha) != 1.0f) {
#else
    // if (self.hudView.alpha != 1.0f) {
    if (self.foregroundHudView.alpha != 1.0f) {
#endif
        ...
        self.foregroundHudView.transform = CGAffineTransformScale(self.foregroundHudView.transform, 1.3, 1.3);
...
// as we cannot animate this property and use alpha values instead
        ...
        if (self.defaultStyle != SVProgressHUDStyleCustom && !greateriOS9 && !_flatHud) {
            [self addBlur];
        ...
        __block void (^animationsBlock)(void) = ^{
            ...
            // self.hudView.transform = CGAffineTransformScale(self.hudView.transform, 1 / 1.3f, 1 /
            // 1.3f);
            self.foregroundHudView.transform =
            CGAffineTransformScale(self.foregroundHudView.transform, 1 / 1.3f, 1 / 1.3f);
            ...
            if (self.defaultStyle != SVProgressHUDStyleCustom && !_flatHud) {
                ...
                self.foregroundHudView.alpha = 1.f;
            }
#else
            self.hudView.alpha = 1.0f;
            self.hudView.contentView.alpha = 1.0f;
        ...
        __block void (^completionBlock)(void) = ^{
            ...
            // and the change of these values has not been cancelled in between e.g. due to a
dismissal
            // Checking one alpha value is sufficient as they are all the same
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if (strongSelf) {
            ...
            __block void (^animationsBlock)(void) = ^{
                ...
                // CGAffineTransformScale(strongSelf.hudView.transform, 1 / 1.3f, 1 / 1.3f);
                strongSelf.foregroundHudView.transform =
                CGAffineTransformScale(strongSelf.foregroundHudView.transform, 1 / 1.3f, 1 / 1.3f);
                ...
                if (self.defaultStyle != SVProgressHUDStyleCustom && !_flatHud) {
                    ...
                    strongSelf.foregroundHudView.alpha = 0.0f;
                }
#else
                strongSelf.hudView.alpha = 0.0f;
                strongSelf.hudView.contentView.alpha = 0.0f;
...
// Checking one alpha value is sufficient as they are all the same
                ...
                if (strongSelf.hudView.alpha == 0.0f) {
                    ...
                    [strongSelf.foregroundHudView removeFromSuperview];
...
keyWindow].rootViewController;
            ...
            // UIViewAnimationOptionBeginFromCurrentState AND a delay doesn't always work as
            // expected
            // When UIViewAnimationOptionBeginFromCurrentState is set, animateWithDuration:
            // evaluates the current
            // values to check if an animation is necessary. The evaluation happens at function call
            // time and not
            // after the delay => the animation is sometimes skipped. Therefore we delay using
            // dispatch_after.
...
- (UIView *)indefiniteAnimatedView {
    ...
    if (self.defaultAnimationType == SVProgressHUDAnimationTypeFlat) {
        ...
        if (_indefiniteAnimatedView &&
            ![_indefiniteAnimatedView isKindOfClass:[SVIndefiniteAnimatedView class]]) {
...
- (void)cancelRingLayerAnimation {
    ...
    [self.foregroundHudView.layer removeAllAnimations];
...
/*
 ...
 CGFloat minimum = MAX((CGFloat)string.length * 0.06 + 0.5, [self
 sharedView].minimumDismissTimeInterval);
...
- (UIControl *)controlView {
    if (!_controlView) {
        ...
        [_controlView addTarget:self
                         action:@selector(controlViewDidReceiveTouchEvent:forEvent:)
...
- (UIView *)backgroundView {
    ...
    if (!_backgroundView.superview) {
        ...
        [self insertSubview:_backgroundView belowSubview:self.foregroundHudView];
    }
...
- (UIVisualEffectView *)hudVibrancyView {
    if (!_hudVibrancyView) {
        ...
        _hudView.layer.masksToBounds = YES;
        _hudVibrancyView.autoresizingMask =
        UIViewAutoresizingFlexibleBottomMargin | UIViewAutoresizingFlexibleTopMargin |
        UIViewAutoresizingFlexibleRightMargin | UIViewAutoresizingFlexibleLeftMargin;
    }
...
- (UILabel *)statusLabel {
    ...
    if (!_statusLabel.superview) {
        ...
        if (!_flatHud) {
            ...
            [self.foregroundHudView addSubview:_statusLabel];
        }
#else
...
- (UIImageView *)imageView {
    ...
    if (!_imageView) {
        ...
        initWithFrame:CGRectMake(0.0f, 0.0f, _imageViewSize.width, _imageViewSize.height)];
    }
    if (!_imageView.superview) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000
        //[self.hudVibrancyView.contentView addSubview:_imageView];
        if (_flatHud) {
            [self.foregroundHudView addSubview:_imageView];
        } else {
            [self.hudVibrancyView.contentView addSubview:_imageView];
        }
#else
...
- (UIWindow *)frontWindow {
    ...
    for (UIWindow *window in frontToBackWindows) {
        ...
        (window.windowLevel >= UIWindowLevelNormal && window.windowLevel <= self.maxSupportedWindowLevel);
...
- (void)addBlur {
    ...
    self.defaultStyle == SVProgressHUDStyleDark ? UIBlurEffectStyleDark : UIBlurEffectStyleExtraLight;
...
- (void)dismiss:(BOOL)animated {
    self.fadeOutAnimationDuration = animated ? SVProgressHUDDefaultAnimationDuration : 0.f;
    [self dismissWithDelay:0.f completion:nil];
}

- (UIView *)foregroundHudView {
    return _flatHud ? self.flatHudView : self.hudView;
}

- (UIView *)flatHudView {
    if (!_flatHudView) {
        _flatHudView = [UIView new];
        _flatHudView.layer.masksToBounds = YES;
        _flatHudView.autoresizingMask =
        UIViewAutoresizingFlexibleBottomMargin | UIViewAutoresizingFlexibleTopMargin |
        UIViewAutoresizingFlexibleRightMargin | UIViewAutoresizingFlexibleLeftMargin;
    }
    if (!_flatHudView.superview) {
        [self addSubview:_flatHudView];
    }

    _flatHudView.backgroundColor = self.backgroundColorForStyle;

    return _flatHudView;
}

- (void)setFlatHud:(BOOL)flatHud {
    if (!_isInitializing)
        _flatHud = flatHud;
}
