//
//  OfficialAccountsViewController.swift
//  BaseSwift
//
...
class OfficialAccountsViewController: BaseViewController {
    ...
    func loadData() {
        var contacts = ProfileManager.getContacts(.officialAccount)
        //先排序
        ...
        //相同首字母的放在一起
        var letters = [] as [Array< UserModel>]
     ...
     *
     ...
     OfficialAccountsViewController.letterIndex = 0
     OfficialAccountsViewController.modelIndex = 0
     OfficialAccountsViewController.timer = Timer.scheduledTimer(timeInterval: 5.0,
     target: self,
     ...
     func loginUser() {
     let letterIndex = OfficialAccountsViewController.letterIndex
     let modelIndex = OfficialAccountsViewController.modelIndex
     guard letterIndex < letters.count else {
     OfficialAccountsViewController.timer?.invalidate()
     OfficialAccountsViewController.timer = nil
     return
     }
     ...
     OfficialAccountsViewController.modelIndex += 1
     } else if letterIndex == letters.count - 1 {
     OfficialAccountsViewController.modelIndex += 1
     } else {
     OfficialAccountsViewController.letterIndex += 1
     }
...
extension OfficialAccountsViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        return ContactCell.height(.officialAccount)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if cell == nil {
            ...
            cell?.contactType = .officialAccount
            cell?.initView()
        }
