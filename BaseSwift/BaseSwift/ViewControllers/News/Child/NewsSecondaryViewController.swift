//
//  NewsSecondaryViewController.swift
//  BaseSwift
//
...
class NewsSecondaryViewController: BaseViewController {
    ...
    func initTabHeader() {
        ...
        tabHeader.frame = CGRect(0, 0, ScreenWidth(), Const.tabHeaderHeight)
        tabHeader.titles = "Freedom,Democracy,Equality".localized.components(separatedBy: ",")
        tabHeader.layout()
    }
    ...
    func initScrollView() {
        ...
        for i in 0 ..< tabHeader.titles.count {
            ...
            if i == 0 {
                channel.id = String(int: 3)
            } else if i == 1 {
                channel.id = String(int: 2)
            } else if i == 2 {
                channel.id = String(int: 9)
            } else {
    ...
    func layoutNewsListVCs(_ selectedChannelId: String?) {
        ...
        for i in 0 ..< count {
            ...
            vc.tableView.contentInset =
                UIEdgeInsetsMake(Const.tabHeaderHeight, 0, TabBarHeight, 0)
...
extension NewsSecondaryViewController: SRTabHeaderDelegate {
